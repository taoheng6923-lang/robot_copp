...
def plot_splp_result(
    ...
    # ⑥ 关节速度 q̇_i(t)（区间内细剖面重构，与加速度导数自洽）
    rec = reconstruct_time_signals(data, profile)
    for i in range(data.n_axis):
        ax[1, 2].plot(rec["t"], rec["qd"][i], label=f"关节 {i}")
    for v in (data.vmax.max(), -data.vmax.max()):
        ax[1, 2].axhline(v, ls=":", color="0.6", lw=0.8)
    ax[1, 2].set_xlabel("时间 $t$ (s)"); ax[1, 2].set_ylabel(r"$\dot q_i$")
    ax[1, 2].set_title(r"⑥ 关节速度 $\dot q_i(t)$")
...
def plot_kinematic_limits(
    ...
    def _joint_panel(axis, sig_key, limit, name, sym):
        for i in range(data.n_axis):
            axis.plot(t, rec[sig_key][i], label=f"关节 {i}", lw=1.2)
        lim = float(np.max(limit))
        axis.axhline(lim, ls="--", color="0.4", lw=1.0)
        axis.axhline(-lim, ls="--", color="0.4", lw=1.0, label="约束")
        axis.set_xlabel("时间 $t$ (s)"); axis.set_ylabel(sym)
        axis.set_title(name); axis.legend(fontsize=8, ncol=2)
        axis.set_ylim(-1.25 * lim, 1.25 * lim)
    ...
    # ④ 关节力矩 τ_i(t)（M4；上下界可非对称）
    if data.torque is not None and "torque" in rec:
        tmax = float(np.max(data.torque.tau_max))
        tmin = float(np.min(data.torque.tau_min))
        for i in range(data.n_axis):
            ax[1, 0].plot(t, rec["torque"][i], label=f"关节 {i}", lw=1.2)
        ax[1, 0].axhline(tmax, ls="--", color="0.4", lw=1.0)
        ax[1, 0].axhline(tmin, ls="--", color="0.4", lw=1.0, label="约束")
        ax[1, 0].set_xlabel("时间 $t$ (s)"); ax[1, 0].set_ylabel(r"$\tau$")
        ax[1, 0].set_title(r"④ 关节力矩 $\tau_i$"); ax[1, 0].legend(fontsize=8, ncol=2)
    ...
    fig.text(
        0.5, 0.015,
        "注：TCP 曲线来自合成解析 TCP 路径、力矩系数来自合成惯性+重力"
        "（M1/M4 无真实运动学/动力学；实际由 Jacobian / 逆动力学给出，见设计 §5）。"
