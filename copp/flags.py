...
每类约束都可单独启用/关闭；开关设置放在全局参数文件 configs/comm_paras.yaml 的
`constraints` 节，经 config.load_constraint_flags() 读取，传入 SolveOptions.flags。

关闭某约束仅表示求解时不施加它（其上/下界数据仍可存在，只是不生效）。
"""
