...
>
> **与实际实现的差异**：本文档 §2 的目录树把所有模块放在单一 `copp/` 包下（早期设计快照）；
> 实际实现是顶层 `robot/`（运动学动力学本体，独立）+ `trajectory-planning/` 目录下的
> `copp/`（纯数值核心）、`path/`（commands/blending/lowering，即本文档的指令层+blending+
> lowering）、`planner/`（hlaw/synth/planner.py）三个包。模块职责、算法原理与接口骨架仍按
> 本文档描述，只是包的挂载位置不同。以实际代码结构为准时请参照 [`README_M1.md`](./README_M1.md)
> 顶部说明。
