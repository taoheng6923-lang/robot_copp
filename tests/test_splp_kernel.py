...
可用 pytest 运行，也可直接 `python tests/test_splp_kernel.py`。
"""
...
from copp import (
    Topp3Data, SyntheticRobotModel, ConstraintFlags, solve_splp, SolveOptions,
...
# 机器人运动学/动力学计算集中于 SyntheticRobotModel（M1 stand-in）
_MODEL = SyntheticRobotModel(n_axis=3, seed=3)


def _make_data(n_grid=81) -> Topp3Data:
    """合成数据（M4：TCP 速度模 + 关节力矩），本体量全部来自 SyntheticRobotModel。"""
